...
ggplot(joined_icu_chart_id, mapping = aes(x = charttime, y = valuenum, 
  ...
  geom_point() +
  geom_line() +
  facet_grid(abbreviation~stay_id, scales="free", space="free_x") +
...
#Mutate storetime as dttm for comparison to intime
labevents_tble <- labevents_tble %>% mutate(storetime = 
  as.POSIXct(storetime, tz = "UTC")) 

#Filtering where storetime < intime (of ICU stay)
labevents_tble <- labevents_tble %>%  filter(storetime < intime) %>%
...
#We join labevents_tble with d_labitems by item_id to get the labels for the items 
d_labitems <- read_csv("~/mimic/hosp/d_labitems.csv.gz")
labevents_tble <- labevents_tble %>% left_join(d_labitems, by = "itemid") %>% 
...
chartevents_tble <- arrow::open_dataset("~/chartevents_pq") %>% 
  ...
  #Mutate subject_id, hadm_id, stay_id as double to join with icustays_tble
  mutate(subject_id = as.double(subject_id)) %>%
...
#Mutate storetime as dttm for comparison to intime
chartevents_tble <- chartevents_tble %>% mutate(storetime = 
  as.POSIXct(storetime, tz = "UTC")) 

#Filter by measurements within the ICU stay
chartevents_tble <- chartevents_tble %>% filter((storetime > intime)
  & (storetime < outtime)) %>% 
  #We group by stay_id and itemid to get the values for each measurement
  #of each stay 
  group_by(stay_id, itemid) %>%
...
#We join labevents_tble with d_items by item_id to get the labels for the items 
d_items <- read_csv("~/mimic/icu/d_items.csv.gz")
chartevents_tble <- chartevents_tble %>% left_join(d_items, by = "itemid")

#We subset chartevents_tble to only the columns we need in our final result

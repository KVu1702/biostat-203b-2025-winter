...
procedures_icd <- read_csv("~/mimic/hosp/procedures_icd.csv.gz") %>% 
   filter(subject_id %in% ID)
d_procedures_icd <- read_csv("~/mimic/hosp/d_icd_procedures.csv.gz") 

#Joining the two data frames by icd_code to get the procedure name
#We only want the chartdate and long_title 
joined_procedures <- left_join(procedures_icd, d_procedures_icd, 
                               by = "icd_code") %>% 
  select(all_of(c("chartdate", "long_title"))) %>%
  rename("Calender_Time" = chartdate) %>%
  mutate(type = "Lab") 

#Reading in admissions
admissions <-
...
res_df <- res_df %>% mutate(long_title = str_wrap(long_title, 10))
...
ggplot() + 
  ...
  ) + xlab("Calender Time") + labs(shape = "Procedure")

...
param_grid_gb <- grid_regular(
  ...
  learn_rate(range = c(1,2), trans = log10_trans()),
  levels = c(3, 2)
  )
...
#Creating the 5-folds
folds_gb <- vfold_cv(mimiciv_icu_cohort, v = 5)
...
```{r}
#SVM
#Using code from the lecture notes, defining the SVM recipe
svm_recipe <- 
  recipe(
    los_long ~ ., 
    data = mimiciv_icu_cohort
...
#Creating the SVM model with the parameters we want to tune
svm_mod <- 
  svm_rbf(
    mode = "classification",
    cost = tune(),
...
#Creating the SVM workflow with the recipe and model
svm_wf <- workflow() |>
...
#Setting seed for 2-fold CV
set.seed(203)
#Creating the 2-folds
folds_svm <- vfold_cv(mimiciv_icu_cohort, v = 2)

#Fitting the SVM model with the tunable parameters with the
#metrics of roc/auc and accuracy
svm_fit <- svm_wf |>
  tune_grid(
    resamples = folds_svm,
...
#Creating the final workflow with the best SVM model
final_wf_svm <- svm_wf |>
...
#Collecting the metrics of the best SVM model.
final_fit_svm |> 
  collect_metrics()
```
...
#We set up the folds shared across all models
#We use 2-fold CV for computational efficiency
set.seed(203)
folds_stack <- vfold_cv(Heart_other, v = 2)
...
#Generating predictions from each of the ensemble members
los_pred <-
  los_test |>
  select(los_long) |>
  bind_cols(
